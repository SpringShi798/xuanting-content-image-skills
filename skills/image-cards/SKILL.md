...
metadata:
  openclaw:
    homepage: https://github.com/JimLiu/baoyu-skills#image-cards
---
...
3. **Auto-select** (when the preference is `auto`, unset, or the pinned backend isn't available):
   ...
   - Otherwise, if exactly one non-native backend is installed (e.g., `imagine`), use it.
...
| Path | Scope |
|------|-------|
| `.xuanting-skills/image-cards/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/xuanting-skills/image-cards/EXTEND.md` | XDG |
| `$HOME/.xuanting-skills/image-cards/EXTEND.md` | User home |
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure image-cards preferences" / "重新配置"). The next run re-triggers first-time setup.
- **Common one-line edits**:
  - `preferred_image_backend: auto` — default; runtime-native tool wins, falls back to the only installed backend, asks only if multiple non-native are present.
  - `preferred_image_backend: codex-imagegen` — pin to Codex's built-in.
  - `preferred_image_backend: imagine` — pin to the imagine skill.
