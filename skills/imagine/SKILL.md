...
metadata:
  openclaw:
    homepage: https://github.com/JimLiu/baoyu-skills#imagine
    requires:
      anyBins:
        - bun
        - npx
...
| Input is `outline.md` + `prompts/` (e.g. from `article-illustrator`) | Batch — use `scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
