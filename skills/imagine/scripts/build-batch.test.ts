...
const scriptPath = path.join(repoRoot, "skills", "imagine", "scripts", "build-batch.ts");
...
async function makeFixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "imagine-build-batch-"));
...
test("build-batch omits default model so imagine can resolve env or EXTEND defaults", async () => {
