...
test("path normalization, worker count, and retry classification follow expected rules", () => {
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("Replicate returned 2 outputs, but imagine currently supports saving exactly one image per request."),
    ),
    false,
  );
  assert.equal(
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("DashScope wan2.7 image models in imagine support exactly one output image per request."),
    ),
    false,
  );
  assert.equal(
