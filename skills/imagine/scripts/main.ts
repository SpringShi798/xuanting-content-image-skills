...
const DEFAULT_PROVIDER_RATE_LIMITS: Record<Provider, ProviderRateLimit> = {
  ...
  grsai: { concurrency: 2, startIntervalMs: 1500 },
};
...
Options:
  ...
  --provider google|openai|openrouter|dashscope|zai|minimax|replicate|jimeng|seedream|azure|grsai  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  GRSAI_API_KEY             grsai (nano-banana 系列) API key — see https://grsai.ai/zh/dashboard/api-keys
  GRSAI_BASE_URL            grsai endpoint base (default: https://grsaiapi.com)
  GRSAI_IMAGE_MODEL         Default grsai model (default: nano-banana-pro)
...
export function getConfiguredProviderRateLimits(
  ...
  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "zai", "minimax", "jimeng", "seedream", "azure", "grsai"] as Provider[]) {
...
function inferProviderFromModel(model: string | null): Provider | null {
  ...
  if (normalized.startsWith("nano-banana")) return "grsai";
  return null;
}
...
export function detectProvider(args: CliArgs): Provider {
  ...
  const hasGrsai = !!process.env.GRSAI_API_KEY;
  ...
  if (modelProvider === "grsai") {
    if (!hasGrsai) {
      throw new Error("Model looks like a grsai (nano-banana) model, but GRSAI_API_KEY is not set.");
    }
    return "grsai";
  }
  ...
  if (args.referenceImages.length > 0) {
    ...
    if (hasGrsai) return "grsai";
    if (hasReplicate) return "replicate";
    if (hasSeedream) return "seedream";
    if (hasMinimax) return "minimax";
    throw new Error(
      "Reference images require Google, OpenAI, Azure, OpenRouter, grsai, Replicate, supported Seedream models, or MiniMax. Set GOOGLE_API_KEY/GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, GRSAI_API_KEY, REPLICATE_API_TOKEN, ARK_API_KEY, or MINIMAX_API_KEY, or remove --ref."
    );
  }
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "grsai") return (await import("./providers/grsai")) as ProviderModule;
