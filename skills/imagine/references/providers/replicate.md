...
Read when the user picks `--provider replicate`. Replicate support is intentionally scoped to model families imagine can validate locally and save without dropping outputs.
...
- If a model is outside the compatibility list above, imagine treats it as prompt-only and rejects advanced local options instead of guessing a nano-banana-style schema
